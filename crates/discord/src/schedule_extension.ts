// Embedded in the pico worker binary and written to `<root>/schedule/extension.ts`
// at startup; loaded via `omp --extension` for every session. Registers the
// schedule_* tools that let the model create and manage scheduled jobs. Each tool
// shells out to the `pico schedule` CLI, which performs all validation and db
// writes. The per-turn context (platform/guild/channel/thread/user) is supplied by
// pico via the inherited PICO_* env — the model only chooses the job's own fields.
//
// Self-contained on purpose (no imports): runs in omp's bun runtime with global
// Bun/Response.

export default function schedule(pi) {
  const z = pi.zod;
  ...
  pi.registerTool({
    ...
    description: [
      ...
      "Do NOT pass platform/guild/channel/user context: pico injects it from the environment.",
    ].join(""),
    parameters: z.object({
    ...
    async execute(_id, p) {
      const payload = { name: p.name, mode: p.mode, trigger: p.trigger };
      if (p.script !== undefined) payload.script = p.script;
      if (p.prompt !== undefined) payload.prompt = p.prompt;
      if (p.target_channel !== undefined) payload.target = p.target_channel;
  ...
  pi.registerTool({
    ...
    parameters: z.object({}),
    async execute() {
      const items = await createOrList(["list", "--json"]);
