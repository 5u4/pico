...
- append pico identity and platform context through the omp sdk; do not replace its system prompt, so omp's coding instructions remain intact
  - identity files load in order: `<picoRoot>/agents/identity.md`, `agents/discord/bots/{botId}/identity.md`, then `agents/discord/channels/{channelId}/identity.md`
  ...
  - native workspaces load only the global file; channel identity is shared across bots
  ...
  - missing and blank identity files are ignored; other read failures include the path and cause
  - all layers append; empty files do not clear inherited content; more-specific identity conventions take precedence as model guidance, not authorization
  ...
  - config owns identity file paths and reads; application session context owns scope selection and prompt composition; OMP receives the completed text
- register bundled `pico-schedule` and `pico-identity` through per-session OMP `skills.customDirectories`; append after configured directories without copying into user settings
