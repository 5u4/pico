...
# the local `.env.e2e` secrets and `.cargo` config, so a worktree agent can't
# run the e2e tests. Symlink those local files from the main worktree into each
# new one. They stay gitignored and uncommitted (`.env*` and `.cargo` are both
# ignored) — only this script, its hook, and its test are tracked, so nothing
# private enters git history.
#
