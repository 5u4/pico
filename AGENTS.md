...
This applies on top of the "commit messages matter" rule above: the message must still be specific about what actually changed.

## Language

Split by destination, not by topic:

- **English** — everything that lands in the repo, git history, or GitHub:
  reasoning/thinking, code, identifiers, log and error strings, test
  names, commit messages, and PR titles/bodies. Anyone contributing reads it,
  so it stays English and consistent.
- **中文** — the assistant's chat replies to the maintainer in this
  conversation, and nothing else.

So: think and write the work in English; address the maintainer in Chinese.
