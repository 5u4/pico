...
1. **Worktree fork** (`crates/core/src/worktree.rs`): the branch name is always `format!("{branch_prefix}/{thread_id}")` (`worktree.rs:17-19`, default prefix `"pico"` from `bindings.rs:24`); the path is `worktrees_dir/<platform>/<safe(channel_id)>/<thread_id>` (`worktree.rs:34-39`; `safe_component` maps path separators/colons to `_`, `worktree.rs:21-32`). `ensure_at` (`worktree.rs:55-119`) is **idempotent** — it checks whether `.git` already exists first (`worktree.rs:63-65`) — prunes stale worktree entries, best-effort fetches `origin` only if `default_branch` starts with `"origin/"` (warns and continues on failure, `worktree.rs:81-85`), then either `git worktree add <path> <existing-branch>` or `git worktree add -b <branch> <path> <default_branch>` (`worktree.rs:87-116`). Because it reuses the branch across restarts, resuming a thread after a worker restart lands back on the exact same branch instead of forking a new one.
...
2. `worktree::close_would_lose(path, branch)` runs `git status --porcelain` and `git rev-list --count <branch> --not --remotes HEAD`.
